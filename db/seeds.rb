...
def getPrice(ingredient)
  ...
  if (doc["results"][0].nil? || doc["results"][0]["items"][0]["name"] == "ASDA Finely Sliced Honey Roast Dry Cured Ham") && ingredient.include?("+")
    getPrice(ingredient.split("+").sort_by {|x| x.length}.last)
  elsif doc["results"][0].nil? || doc["results"][0]["items"][0]["name"] == "ASDA Finely Sliced Honey Roast Dry Cured Ham"
    return 0
  end
